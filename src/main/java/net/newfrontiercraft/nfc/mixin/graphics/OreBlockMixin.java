...
public class OreBlockMixin implements CustomTooltipProvider {
    @Override
    public @NotNull String[] getTooltip(ItemStack stack, String originalTooltip) {
        ToolTierEnum toolTierEnum;
        Item item = stack.getItem();
        if (item == Block.GOLD_ORE.asItem()) {
            toolTierEnum = ToolTierEnum.WOOD;
        } else if (item == Block.IRON_ORE.asItem()) {
            toolTierEnum = ToolTierEnum.IRON;
        } else if (item == Block.DIAMOND_ORE.asItem()) {
